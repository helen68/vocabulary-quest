...
window.addEventListener("DOMContentLoaded", () => {
    ...
    // 預熱語音引擎 (Crucial for Android Chrome lazy voice load)
...
function speakText(txt) {
    if ("speechSynthesis" in window) {
        // 1. Cancel any current speech queue to reset a potentially stuck state in Android Chrome
        window.speechSynthesis.cancel();

        const utter = new SpeechSynthesisUtterance(txt);
        utter.rate = 0.85;

        // 2. Query available voices asynchronously loaded by the browser
        const voices = window.speechSynthesis.getVoices();
        if (voices && voices.length > 0) {
            // Find best fit English voice (local voices preferred to avoid network latency)
            let voice = voices.find(v => v.lang.toLowerCase() === "en-us" && v.localService);
            if (!voice) {
                voice = voices.find(v => v.lang.toLowerCase() === "en-us");
            }
            if (!voice) {
                voice = voices.find(v => v.lang.toLowerCase().startsWith("en-") && v.localService);
            }
            if (!voice) {
        ...
        // 3. Play sound synchronously within the direct call stack of user event handling
